...
import { useDeleteRuleType } from "@/hooks/useDeleteRuleType";
...
export function EditRuleTypeActionsCell({ ruleSetId }: Props) {
    ...
    const deleteRule = useDeleteRuleType();
    ...
    const handleDelete = () => {
        deleteRule.mutate(ruleSetId, {
            onSuccess: () => {
                setOpen(false);
                toast.success(`Sampling Rule #${ruleSetId} deleted successfully.`);
            },
            onError: (error) => {
