 ...
 *
 *  Server-ranked WO×step rows. When the APS schedule has placed a row, the queue
 *  follows the plan (scheduled rows lead, ordered by `scheduled_start`); rows the
 *  solver hasn't placed fall back to the WO priority → due → aging heuristic.
 *  `is_held` rows sink into a 'blocked' bucket. v1 readiness = ready | blocked
