...
type UpdatePartInput = Parameters<typeof api.api_Parts_partial_update>[0];

// 2️⃣ Infer the shape of the `params` object:
type UpdatePartConfig = Parameters<typeof api.api_Parts_partial_update>[1];
type UpdatePartParams = UpdatePartConfig["params"];

// 3️⃣ Infer the response type, if you need it:
type UpdatePartResponse = Awaited<ReturnType<typeof api.api_Parts_partial_update>>;
    ...
    return useMutation<UpdatePartResponse, unknown, UpdatePartVariables>({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({
                queryKey: ["parts"],
                predicate: (query) => query.queryKey[0] === "parts",
            });
        },
    });
};
