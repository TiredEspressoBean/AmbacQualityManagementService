...
type UpdateProcessInput = Parameters<typeof api.api_Processes_with_steps_partial_update>[0];

// 2️⃣ Infer the shape of the `params` object:
type UpdateProcessConfig = Parameters<typeof api.api_Processes_with_steps_partial_update>[1];
type UpdateProcessParams = UpdateProcessConfig["params"];

// 3️⃣ Infer the response type, if you need it:
type UpdateProcessResponse = Awaited<ReturnType<typeof api.api_Processes_with_steps_partial_update>>;
...
export const useUpdateProcess = () => {
    ...
    return useMutation<UpdateProcessResponse, unknown, UpdateProcessVariables>({
        mutationFn: ({ id, data }) =>
            api.api_Processes_with_steps_partial_update(data, {
                params: { id },
