...
def _positions_for(core_type, component_type, quantity: int) -> list:
    ...
    line = DisassemblyBOMLine.objects.filter(
...
def _harvested_by_position(core):
    ...
    out = {}
    loose = {}
    for hc in core.harvested_components.filter(is_scrapped=False).select_related('component_type'):
...
def _pool_candidates(component_type, core):
    ...
    if not core.allows_pooled_harvest:
        return []
    qs = (
        Parts.objects.filter(part_type=component_type, reserved_for_core__isnull=True)
...
def _purchased_candidates(component_type):
    ...
    qs = MaterialLot.objects.filter(
        material_type=component_type, quantity_remaining__gt=0,
...
def resolve_rebuild_plan(core) -> RebuildPlan:
    ...
    if bom is None:
        return RebuildPlan(
            ...
            fulfilment_mode=core.fulfilment_mode, bom_revision=None, slots=[],
            warnings=[
    ...
    warnings.append(
        "Scope resolution is not built yet, so this is the whole released BOM rather "
        "than the operations this unit actually needs. Expect it to over-state."
    )
    ...
    slots = []
    lines = bom.lines.select_related('component_type').order_by('line_number')
    for line in lines:
    ...
    return RebuildPlan(
        ...
        bom_revision=bom.revision, slots=slots, warnings=warnings,
    )
