...
from Tracker.models import (
    ...
    EquipmentUsage, StepTransitionLog, WorkOrder, SamplingRuleSet, SamplingRule,
    MeasurementDefinition, MeasurementResult, QaApproval, PartsStatus, OrdersStatus,
    SamplingTriggerState, SamplingAuditLog, SamplingAnalytics, MeasurementDisposition,
    ExternalAPIOrderIdentifier, WorkOrderStatus, ClassificationLevel
)
import random
...
class Command(BaseCommand):
    ...
    def create_realistic_groups(self):
        ...
        groups_data = [
            ('Customer', 'External customers who place orders'),
            ('Operator', 'Production floor workers and technicians'),
            ('Manager', 'Production and operations managers'), 
    ...
    def create_injector_part_types_and_processes(self, employees):
                ...
                'processes': [
                    {
                        ...
                        'is_reman': True,
                        'steps': [
                            'Initial Inspection', 'Disassembly', 'Parts Cleaning', 
                            'Component Testing', 'Wear Assessment', 'Parts Replacement',
                            'Assembly', 'Calibration', 'Flow Testing', 'Final QC'
                        ]
                    },
                    {
                        'name': 'New Manufacturing Process',
                        'is_reman': False,
                        'steps': [
                            'Material Receiving', 'Component Fabrication', 'Precision Machining',
                            'Assembly', 'Initial Testing', 'Calibration', 'Flow Testing', 'Final QC'
                        ]
                    }
                ]
            },
                ...
                'processes': [
                    {
                        ...
                        'is_reman': True,
                        'steps': [
                            'Intake Inspection', 'Disassembly', 'Ultrasonic Cleaning',
                            'Pressure Testing', 'Nozzle Replacement', 'Reassembly',
                            'Electronic Testing', 'Final Validation'
                        ]
                    },
                    {
                        'name': 'New Manufacturing Process',
                        'is_reman': False,
                        'steps': [
                            'Raw Material Prep', 'Component Manufacturing', 'Quality Inspection',
                            'Assembly', 'Electronic Testing', 'Performance Testing', 'Final QC'
                        ]
                    }
                ]
            },
                ...
                'processes': [
                    {
                        ...
                        'is_reman': True,
                        'steps': [
                            'Visual Inspection', 'Teardown', 'Component Cleaning',
                            'Hydraulic Testing', 'Electrical Testing', 'Rebuild',
                            'Performance Testing', 'Quality Verification'
                        ]
                    },
                    {
                        'name': 'New Manufacturing Process',
                        'is_reman': False,
                        'steps': [
                            'Material Sourcing', 'Component Fabrication', 'Hydraulic Assembly',
                            'Electrical Assembly', 'System Testing', 'Final QC'
                        ]
                    }
                ]
            }
        ...
        for pt_data in part_type_data:
            part_type = PartTypes.objects.create(
                name=pt_data['name'],
                ID_prefix=pt_data['prefix']
            )
            ...
            for proc_data in pt_data['processes']:
                ...
                process = Processes.objects.create(
                    ...
                    part_type=part_type,
                    num_steps=len(proc_data['steps']),
                    is_batch_process=is_batch
                )
    ...
    def create_realistic_orders(self, companies, customers, config):
        ...
        status_weights = [
            (OrdersStatus.COMPLETED, 0.15),     # 15% completed
        ...
        for i in range(config['orders']):
            ...
            order_date = self.fake.date_between(start_date=start_date, end_date=timezone.now().date() - timedelta(days=7))
    ...
    def create_parts_with_workflows(self, orders, part_types, users, equipment, config):
        ...
        for order in orders:
            ...
            for wo_index in range(num_work_orders):
                ...
                work_order_details = self.determine_work_order_details(order, order_part_type, order_process, wo_index, config)
    ...
    def get_realistic_sampling_rules_for_step(self, step):
        ...
        if any(word in step_name for word in ['inspection', 'qc', 'validation', 'final', 'verification']):
            ...
            return [
                {'rule_type': 'every_nth_part', 'value': random.randint(2, 4), 'order': 1}  # Every 2-4 parts
            ]
        elif any(word in step_name for word in ['testing', 'flow', 'pressure', 'hydraulic', 'electronic', 'performance']):
            # Testing steps - moderate sampling
            return [
                {'rule_type': 'percentage', 'value': random.randint(25, 50), 'order': 1}  # 25-50% of parts
            ]
        elif any(word in step_name for word in ['assessment', 'calibration']):
            # Assessment/calibration steps - moderate sampling
            return [
                {'rule_type': 'every_nth_part', 'value': random.randint(3, 6), 'order': 1}  # Every 3-6 parts
            ]
            ...
            return [
                {'rule_type': 'every_nth_part', 'value': random.randint(5, 8), 'order': 1}  # Every 5-8 parts
            ]
        elif any(word in step_name for word in ['cleaning', 'replacement', 'teardown', 'rebuild']):
            # Process steps - light sampling
            return [
                {'rule_type': 'percentage', 'value': random.randint(15, 25), 'order': 1}  # 15-25% of parts
            ]
        else:
            # Default sampling for any other step
            return [
                {'rule_type': 'every_nth_part', 'value': random.randint(6, 10), 'order': 1}  # Every 6-10 parts
            ]
    ...
    def create_quality_report_for_part(self, part, step, users, equipment):
        ...
        qr = QualityReports.objects.create(
            part=part,
            machine=random.choice(equipment) if equipment else None,
            step=step,
        ...
        for measurement_def in step.measurement_definitions.all():
            if measurement_def.type == 'NUMERIC' and measurement_def.nominal:
                ...
                MeasurementResult.objects.create(
                    ...
                    created_by=random.choice(users['employees'])
                )
    ...
    def create_advanced_sampling_data(self, part, users):
        ...
        if (part.work_order and part.sampling_ruleset and 
            part.work_order.workorder_status == WorkOrderStatus.COMPLETED):
    ...
    def create_comprehensive_documents(self, part, users):
        ...
        doc_types = [
            ('Quality Report', ClassificationLevel.INTERNAL, True),
        ...
        for doc_name, classification, is_image in doc_type_sample:
            Documents.objects.create(
                ...
                file=ContentFile(
                    b"Sample document content for " + doc_name.encode(),
    ...
    def create_quality_error_lists_and_dispositions(self, quality_report, part_type, users):
        ...
        if quality_report.status == "FAIL":
            ...
            for _ in range(random.randint(1, 2)):
                ...
                error, created = QualityErrorsList.objects.get_or_create(
                    ...
                    part_type=part_type,
                    defaults={
                        'error_example': f"Example: {error_name} in {part_type.name}"
                    }
                )
                errors.append(error)
        ...
        for measurement in failed_measurements:
            ...
            MeasurementDisposition.objects.create(
                measurement=measurement,
                disposition_type=random.choice(disposition_types),
                resolved_by=random.choice(users['employees']),
