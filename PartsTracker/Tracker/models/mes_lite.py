...
class PartsStatus(models.TextChoices):
    ...
    AT_OUTSIDE_PROCESS = "AT_OUTSIDE_PROCESS", "At Outside Process"  # Sent out to a subcontract vendor (heat treat, plating, …); left the building
...
class PartTypes(SecureModel):
    ...
    requires_supplier_qualification = models.BooleanField(
        default=False,
        help_text="Require an approved SupplierQualification covering the supplier "
                  "before a received lot of this part type can be accepted into stock."
    )
    """When True, receiving soft-holds lots from suppliers without an active qualification."""

    requires_part_approval = models.BooleanField(
        default=False,
        help_text="Require an active PartApproval (PPAP/FAI) covering the "
                  "(part type, supplier) before a received lot of this part type "
                  "can be accepted into stock."
    )
...
# Sentinel: distinguishes "no supplier filter" (default, supplier-agnostic) from an
# explicit supplier=None ("all suppliers" ruleset) in get_resolved_sampling_rules.
_SAMPLING_SUPPLIER_UNSET = object()
...
class Steps(SecureModel):
    ...
    # ===== OUTSIDE PROCESS (subcontract operations — heat treat, plating, …) =====

    is_outside_process = models.BooleanField(
        default=False,
        help_text=(
            "If True, this step is performed by an outside vendor: parts are sent "
            "out (StepExecution send stamps / OutsideProcessShipment) and run a "
            "receiving-style return inspection when they come back. See "
            "services/mes/outside_process.py."
        ),
    )
    """Marks the outbound leg of a subcontract op. The return inspection reuses the
    same DWI receiving-inspection runtime as Flow A (subject = the returned batch)."""

    outside_supplier = models.ForeignKey(
    ...
    def get_resolved_sampling_rules(self, supplier=_SAMPLING_SUPPLIER_UNSET):
        ...
        ``supplier`` scopes which (step, supplier) ruleset to resolve — used by the
        Receiving Inspection Plan editor so per-supplier plans don't collide. Default
        (unset) keeps the supplier-agnostic behavior every other caller relies on."""
        # Get active primary ruleset
        qs = self.sampling_ruleset.filter(is_fallback=False, active=True)
        if supplier is not _SAMPLING_SUPPLIER_UNSET:
            qs = qs.filter(supplier=supplier)
        primary_ruleset = qs.order_by("-version").first()
        ...
        active = {
            ...
            'name': pr.name if pr else None,
            'rules': list(pr.rules.all().values('id', 'rule_type', 'value', 'order')) if pr else [],
            'fallback_duration': pr.fallback_duration if pr else None,
            'tighten_after': (
                int(pr.gate_threshold)
                if pr and pr.gate_metric == 'CONSECUTIVE_FAILS' and pr.gate_threshold is not None else None),
            # Quality gate (for flow-editor prefill).
            'gate_metric': pr.gate_metric if pr else '',
            'gate_threshold': str(pr.gate_threshold) if (pr and pr.gate_threshold is not None) else None,
            'gate_window': pr.gate_window if pr else '',
            'gate_window_n': pr.gate_window_n if pr else None,
            'gate_min_sample': pr.gate_min_sample if pr else None,
            'gate_actions': (pr.gate_actions or []) if pr else [],
            'gate_capa_type': pr.gate_capa_type if pr else '',
            'gate_capa_severity': pr.gate_capa_severity if pr else '',
            'gate_approval_template': str(pr.gate_approval_template_id) if (pr and pr.gate_approval_template_id) else None,
            # Acceptance sampling.
            'aql': str(pr.aql) if (pr and pr.aql is not None) else None,
            'inspection_level': pr.inspection_level if pr else '',
            'severity': pr.severity if pr else '',
            'strategy': pr.strategy if pr else '',
            'variables_characteristic': str(pr.variables_characteristic_id) if (pr and pr.variables_characteristic_id) else None,
        }
        return {'active_ruleset': active,
    ...
    def apply_sampling_rules_update(self, rules_data, process=None, fallback_rules_data=None,
                                    fallback_duration=None, tighten_after=None,
                                    gate=None, acceptance=None, user=None, supplier=None):
        ...
        return apply_step_sampling_rules_update(
            ...
            gate=gate, acceptance=acceptance, supplier=supplier,
        )
...
class StepExecution(SecureModel):
    ...
    # Outside processing: this visit's part was sent out under a shipment (heat
    # treat / plating / grinding). The send/return timestamps + PO reference live
    # on the shipment (single source of truth); this FK records membership. Null
    # for the vast majority of executions (in-house work). See
    # services/mes/outside_process.py.
    outside_process_shipment = models.ForeignKey(
        'Tracker.OutsideProcessShipment', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='step_executions',
        help_text="The subcontract shipment this part left the building under, if any.",
    )
...
class OutsideProcessShipment(SecureModel, VoidableModel):
    """One physical shipment of parts sent out to a subcontract vendor for an
    outside-process operation (heat treat, plating, grinding, …) and received
    back for inspection.

    The aggregate for outside processing (Flow B). It groups the `StepExecution`s
    of the parts that left the building together (one PO / one packing slip), and
    is the AQL lot on return — `quantity` (the linked-execution count) is the
    return-inspection lot size. Send/return timestamps + references live here as
    the single source of truth; the per-part membership is `StepExecution.
    outside_process_shipment`.

    Lifecycle is driven by `services/mes/outside_process.py` (send_parts_out /
    receive_parts_back), never by raw `.save()`.
    """

    # Vendor packing slips, cert of conformance on return, etc.
    documents = GenericRelation('Tracker.Documents')

    SHIPMENT_STATUS_CHOICES = [
        ('SENT', 'Sent Out'),          # parts out at the vendor
        ('RETURNED', 'Returned'),      # back, return inspection open / in progress
        ('CLOSED', 'Closed'),          # return inspection accepted/rejected
    ]

    shipment_number = models.CharField(
        max_length=100, blank=True,
        help_text="Auto-generated, tenant-unique (OSP-YYYY-######).",
    )

    supplier = models.ForeignKey(
        Companies, on_delete=models.PROTECT,
        related_name='outside_process_shipments',
        help_text="The subcontract vendor the parts were sent to.",
    )
    step = models.ForeignKey(
        Steps, on_delete=models.PROTECT,
        related_name='outside_process_shipments',
        help_text="The outside-process step the parts are out for.",
    )
    work_order = models.ForeignKey(
        'Tracker.WorkOrder', null=True, blank=True, on_delete=models.SET_NULL,
        related_name='outside_process_shipments',
        help_text="The work order the shipped parts belong to (usually one).",
    )

    reference = models.CharField(
        max_length=120, blank=True,
        help_text="Outbound PO / packing-slip reference.",
    )
    shipped_at = models.DateTimeField(auto_now_add=True)
    shipped_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='osp_shipments_sent',
    )

    return_reference = models.CharField(
        max_length=120, blank=True,
        help_text="Inbound packing-slip reference on return.",
    )
    returned_at = models.DateTimeField(null=True, blank=True)
    returned_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL,
        related_name='osp_shipments_received',
    )

    status = models.CharField(
        max_length=20, choices=SHIPMENT_STATUS_CHOICES, default='SENT',
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='osp_shipment_tenant_number_uniq',
            ),
        ]

    @property
    def quantity(self):
        """Number of parts in this shipment — the AQL lot size on return."""
        return self.step_executions.count()

    @classmethod
    def generate_shipment_number(cls, tenant=None):
        """Auto-generate shipment number: OSP-YYYY-######."""
    ...
    def __str__(self):
        num = self.shipment_number or 'OSP'
